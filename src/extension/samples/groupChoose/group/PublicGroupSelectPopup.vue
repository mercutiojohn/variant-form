<template>
  <Container direction="vertical" style="height:450px">
    <Row :gutter="20">
      <ELCol :span="12">
    ...
    <Row :gutter="20" style="margin-top: 10px">
      <ELCol :span="12">
        <Form ref='editForm' label-width='150px'>
          <el-scrollbar style="height: 360px;overflow-y: auto;overflow-x: hidden;border: 1px solid #f2f2f2">
      ...
      <ELCol :span="12">
        <el-scrollbar style="height: 360px;overflow-y: auto;overflow-x: hidden;border: 1px solid #f2f2f2">
          ...
          <div v-for="(unit, index) in data" >{{unit.name}}</div>-->
          <div v-for="(domain, index) in checkData" :prop="'domains.' + index + '.name'"
            style="display:inline;margin: 0px 5px;">
            <Button style="margin-bottom: 5px" size="small"
              type="primary" @click="setCheckedKey(domain.id)">{{domain.name}}</Button>
...
<script>
  import { Container, Main, Form, Row, Col, Input, Select, Option, Button, Radio, RadioGroup } from 'element-ui';
