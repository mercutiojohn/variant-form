  ...
  methods: {
    ...
    queryEchartsData(params) {
      return new Promise(async (resolve) => {
        ...
        var dataRes = data;
        if(data.length==1 && data[0].data != undefined && data[0].message!=undefined){
          dataRes = data[0].data
        }
        const analysisData = this.analysisChartsData(params, dataRes);
    ...
    stackChartFn(chartProperties, data) {
      ...
      const stackField = Object.keys(chartProperties)[types.indexOf('stack')]
      ...
      const stackList = data.map(item => item[stackField])
