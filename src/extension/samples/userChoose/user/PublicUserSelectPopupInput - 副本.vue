<template>
  <section>
    ...
    <!-- <div style="display: flex; width:100%; flex-wrap:wrap; gap:10px">
      <el-tag v-for="(item, index) in data" :key="index" size="medium" type="info" style="padding-left:0;display: flex;align-items: center;gap: 2px;">
        <el-avatar icon="el-icon-user-solid" size="small" style="transform: scale(.7)"></el-avatar>
        <span>{{item.name}}</span>
      </el-tag>
      <el-tag v-if="options.disabled && !data.length" size="medium" type="info">无</el-tag>
      <el-button
        @click.native="search"
...
export default {
  ...
  mounted: function () {
    if (this.value && this.value.split("|").length==2) {
      ...
      // for(let i = 0; i < this.userNames.split("、").length; i++) {
      //   this.data.push({
      //     name: this.userNames.split("、")[i],
      //     id: this.userIds.split(",")[i]
      //   })
      // }
    }
  },
